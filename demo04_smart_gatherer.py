...
def move(bot, state):
    ...
    if next_pos not in bot.homezone:
        ...
        for pos in bot.legal_positions:
            # do not go there when an enemy is sitting on it or nearby
            if pos in (enemy[0].position, enemy[1].position):
                continue

            if pos in list(bot.graph[bot.enemy[0].position]):
                continue

            if pos in list(bot.graph[bot.enemy[1].position]):
                continue
